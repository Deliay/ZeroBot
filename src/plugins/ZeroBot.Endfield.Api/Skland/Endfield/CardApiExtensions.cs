...
public static class CardApiExtensions
{
    ...
    {
        public async ValueTask<EndfieldCardDetail> GetEndfieldInfoAsync(SklandUser user, UserAppRole role, UserCredential credential,
            CancellationToken cancellationToken = default)
        {
            const string url = "https://zonai.skland.com/api/v1/game/endfield/card/detail";
            var param = $"roleId={role.roleId}&serverId={role.serverId}&userId={user.id}";
