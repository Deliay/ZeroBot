...
`type=interact` 事件流存在（`cmd == "INTERACT_WORD_V2"`），但 `payload.data.pb` 为 protobuf base64 编码，无现成解析器，实现成本高。按 PRD 约定列入后续迭代。

### 1.4 SSE 订阅

`GET /api/b/live/{roomId}/event/subscribe?type=danmaku` 为 SSE（`text/event-stream`）长连接，按 `data:` 行推送，格式与 `event/range` 的 `items[0]` 一致。解析方式与 `LiveScApi.ReceiveSuperChatEventsAsync` 完全相同，仅事件 DTO 不同。
...
- `RunAsync`：`WaitForInitializedAsync` 后 5s 循环 `Reconcile`，比对 `config.Current.AnchorEventSubscriptions`（聚合出 roomId 集合）与 `_activeRooms`（`ConcurrentDictionary<string, CancellationTokenSource>`，key 为 roomId）：
  - 有订阅未激活 → `CreateLinkedTokenSource` 启动 `ReceiveLoopAsync`（每 roomId 仅一次）；
  - 已激活但无订阅 → `Cancel` 断开。
- `ReceiveLoopAsync(roomId, ct)`：`while (!ct.IsCancellationRequested)` 内 `try { ReceiveDanmakuEventsAsync(...) } catch (OperationCanceledException) { break; } catch { log }` + 5s 退避重连。
- 收到弹幕 → 遍历当前配置中 `RoomId == roomId` 的订阅项，`msg.UserId.ToString() == mid` 时 `AnchorEventMessageBuilder.Build(uName, msg)`，经 `bot.GetAccountInfoAsync` + `WriteManyGroupMessageAsync` 推送给该 mid 的全部订阅群。

#### `Live/AnchorEventMessageBuilder.cs`

```csharp
public static OutgoingSegment[] Build(string anchorName, DanmuMsg msg)
```

- 文本：`{anchorName} 的弹幕 [粉丝牌 等级]：\n{msg.Msg}`；`FansTag` 为空时省略 `[]`。
...
| `src/plugins/ZeroBot.Bilibili/Live/AnchorEventApi.cs` | 新增（用户查询 + danmaku SSE + DTO） |
| `src/plugins/ZeroBot.Bilibili/Live/AnchorEventCommandHandler.cs` | 新增（订阅/取消指令） |
| `src/plugins/ZeroBot.Bilibili/Live/AnchorEventSubscriber.cs` | 新增（reconcile + 接收循环 + 过滤转发） |
| `src/plugins/ZeroBot.Bilibili/Live/AnchorEventMessageBuilder.cs` | 新增（消息构造） |
...
无新增 NuGet 依赖（`Mikibot.Crawler` 已在 `ZeroBot.Bilibili.csproj` 中引用，`DanmuMsg` 属于该包）。

## 4. 关键决策点

1. **主播过滤在转发侧做**：danmaku 流含全量观众弹幕，订阅侧无法按发送者过滤，因此每条事件判断 `UserId == mid`，观众弹幕直接丢弃（判重成本可忽略）。
2. **配置以 mid 为 key、冗余 roomId/uName**：取消订阅与转发均无需再请求接口；SSE 生命周期按聚合出的 roomId reconcile，与 SC 转发一致（每 roomId 一条长连接，空集合即断）。
...
5. **表情包弹幕带图**：`MemeUrl` 非空追加图片段；普通弹幕纯文本，不 at 全体。
6. **入场事件本期不做**：`interact` 事件 payload 为 protobuf，无现成解析器，PRD 已列为后续迭代。
...
| 大订阅量下 SSE 连接数 | 与 SC 转发同量级，每 roomId 一条连接，可接受 |
